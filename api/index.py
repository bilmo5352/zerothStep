...
def get_category_hierarchy(main_category: str) -> dict:
	...
	Returns:
		dict: A dictionary containing the category hierarchy
	"""
	if client is None:
...
# For FastAPI, we need to use ASGI handler
try:
	handler = Mangum(app, lifespan="off")
except Exception as e:
	# If handler creation fails, create a minimal error handler
	...
	async def error_handler():
		return {"error": "Handler initialization failed", "message": str(e)}
